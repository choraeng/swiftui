...
public class CameraViewController: UIViewController {
    ...
    // cho
//    private var croppingFrame: CGRect?
...
//        previewView.preset
    ...
    private func configureSession() {
        ...
        do {
            ...
            if let preferredCameraDevice = AVCaptureDevice.default(preferredStartingCameraType!, for: .video, position: preferredStartingCameraPosition!) {
                ...
                defaultVideoDevice = backCadavemeraDevice
    ...
    private func savePhoto(_ photoData: Data) {
        let inputImage = UIImage(data: photoData)!
        
        let imageViewScale = max(inputImage.size.width / self.croppingFrame!.width,
                                 inputImage.size.height / self.croppingFrame!.height)
        ...
        print(sideLength, sideLength/4*3)
        
        // Scale cropRect to handle images larger than shown-on-screen size
        let cropZone = CGRect(x: 0,
                              y: 0,
                              width: sideLength/3*4, // self.croppingFrame!.width * imageViewScale,
                              height: sideLength) // self.croppingFrame!.height * imageViewScale)
        ...
        if let data = croppedImage.jpegData(compressionQuality: 0.8) {
            ...
            url = URL(fileURLWithPath: tempFile)
        }
...
extension CameraViewController {
    public func setCroppingFrame(frame: CGRect){
        self.croppingFrame = frame
    }
}
